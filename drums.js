/* -------------------------------
   AlgoRhythms — Drums Logic (updated: 32 bars + robust glow)
   ------------------------------- */
...
function ctxInit() {
  ...
  app.an.fftSize = 1024;
  try { app.an.connect(app.ctx.destination); } catch (e) {}
  return app.ctx;
}
...
//   // subtle base radial glow (additive) — robust and visible on dark backgrounds
//   // create a radial centered near bottom middle to give stage-like glow
//   const rg = c2d.createRadialGradient(cw / 2, ch * 0.9, ch * 0.02, cw / 2, ch * 0.9, ch * 0.9);
//   // use lightweight, neon-tinted stops
...
//   c2d.globalCompositeOperation = "source-over";
  ...
  c2d.save();
  c2d.translate(0, ch * 2); // move drawing below canvas
  c2d.scale(1, -1);         // flip vertically

  // draw each mirrored bar faintly
  let xr = gap * 0.5;
  for (let i = 0; i < bars; i++) {
    const barHeight = vizBars[i];
    const y = ch - barHeight;
  ...
  // overlay gradient to fade reflection
  c2d.globalAlpha = 1;
...
/* try init audio ctx + start visuals */
try { ctxInit(); startViz(); } catch (e) {}

/* --- DRUM SYNTHESIS --- */
function playDrum(type, el) {
  const ctx = ctxInit();
  const dest = app.dest, an = app.an;
  ...
  if (type === "kick") {
    ...
    const bufLen = Math.max(1, Math.floor(ctx.sampleRate * 0.16));
    ...
    const noise = ctx.createBufferSource();
    const bufLen = Math.max(1, Math.floor(ctx.sampleRate * 0.05));
    const buffer = ctx.createBuffer(1, bufLen, ctx.sampleRate);
    const data = buffer.getChannelData(0);
    for (let i = 0; i < data.length; i++) data[i] = Math.random() * 2 - 1;
    noise.buffer = buffer;
...
function startRec() {
  ctxInit();
  if (!app.dest) { console.error("No recording destination available"); return; }
...
function tick() {
  ...
  if (recTime) recTime.textContent = `${String(Math.floor(t / 60)).padStart(2, "0")}:${String(t % 60).padStart(2, "0")}`;
...
if (clearAllBtn) clearAllBtn.onclick = () => {
  if (confirm("Clear all recordings?")) { app.recordings = []; renderRecs(); }
};
...
if (dockHeader) {
  dockHeader.onclick = () => {
    ...
    if (dock && dock.classList.contains("expanded") && main) main.scrollTo({ top: 0, behavior: "smooth" });
  };
}
